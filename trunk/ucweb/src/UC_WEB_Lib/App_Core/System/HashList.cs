...
namespace UcentrikWeb.CODE
{
	...
	{
		//array list that contains all the keys as they are inserted, the index is associated witha key so when pulling out values 
        //by index we can get the key for the index, pull from the hashtable the proper value with the corresponding key
		//This is basically the same as a sorted list but does not sort the items, rather it leaves them in the order they were inserted - like a list
		/// <summary>
		/// 
		/// </summary>
